...
void EnumInducedSubtrees::init_graph(Graph* __g)
{
    ...
    // init adjacent lists
    adjacent_lists.reset(
        new std::unique_ptr<AdjacentList>[g->get_sorted_vector().size()]);
    adj_items.reset(new std::unique_ptr<
        std::unique_ptr<AdjItem> []>[g->get_sorted_vector().size()]);
    for (size_t i = 0; i < g->get_sorted_vector().size(); ++i) {
        adj_items[i].reset(
            new std::unique_ptr<AdjItem>[g->get_sorted_vector().size()]);
    }
    ...
    // init candidate list
    cand_items.reset(
        new std::unique_ptr<CandItem>[g->get_sorted_vector().size()]);
    for (const auto& v_id : g->get_sorted_vector()) {
        const Vertex* v = g->get_vertex(v_id);
